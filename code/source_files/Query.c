    ...
    if (word_count == 1) {
        if (DocumentIDs[0].doc_num == 0) {
            ...
            for (int j = 0; j < DocumentIDs[0].doc_num; j++) {
                ...
                printf("Doc ID: %s  Word: %s  Count: %d\n", formatted_filename, DocumentIDs[0].word, DocumentIDs[0].doc_list[j].count);
            }
        }
    } else {
...
void export_precision_and_recall_to_csv_file(int num_entries, expected_result_entry *expected_results, inverted_index_entry *test_documentIDs, int query_type, int threshold){
    // Export the result to a CSV file, the filename is based on the query type and threshold
    char filename[MAX_FILENAME_LEN]; // Adjust size as necessary
    sprintf(filename, "../results/results_with_query_type_%d_threshold_%d.csv", query_type, threshold);
