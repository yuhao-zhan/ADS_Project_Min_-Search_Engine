...
void load_expected_results(const char *filename, expected_result_entry *expected_results, int *num_entries) {
    ...
    json_object_object_get_ex(parsed_json, "one_word", &one_word_array);
    json_object_object_get_ex(parsed_json, "multi_word", &multi_word_array);
    ...
    *num_entries = one_word_count + multi_word_count;
    ...
    for (int i = 0; i < one_word_count; i++) {
        ...
        struct json_object *query, *expected_result_ID, *expected_result_frequency;

        json_object_object_get_ex(entry, "query", &query);
        json_object_object_get_ex(entry, "expected_result_ID", &expected_result_ID);
        json_object_object_get_ex(entry, "expected_result_frequency", &expected_result_frequency);
        ...
        for (int j = 0; j < doc_count; j++) {
            strcpy(expected_results[i].expected_result_ID[j], json_object_get_string(json_object_array_get_idx(expected_result_ID, j)));
            expected_results[i].expected_result_frequency[j] = json_object_get_int(json_object_array_get_idx(expected_result_frequency, j));
        }
    }
    ...
    for (int i = 0; i < multi_word_count; i++) {
        ...
        struct json_object *query, *expected_result_ID, *expected_result_frequency_word1, *expected_result_frequency_word2;

        json_object_object_get_ex(entry, "query", &query);
        json_object_object_get_ex(entry, "expected_result_ID", &expected_result_ID);
        json_object_object_get_ex(entry, "expected_result_frequency_word1", &expected_result_frequency_word1);
        json_object_object_get_ex(entry, "expected_result_frequency_word2", &expected_result_frequency_word2);
        ...
        for (int j = 0; j < doc_count; j++) {
            strcpy(expected_results[one_word_count + i].expected_result_ID[j], json_object_get_string(json_object_array_get_idx(expected_result_ID, j)));

            // For multi-word queries, you need to combine the frequencies of each word
            int freq_word1 = json_object_get_int(json_object_array_get_idx(expected_result_frequency_word1, j));
            int freq_word2 = json_object_get_int(json_object_array_get_idx(expected_result_frequency_word2, j));

            // You can choose how to combine these frequencies. For now, we sum them.
            expected_results[one_word_count + i].expected_result_frequency[j] = freq_word1 + freq_word2;
        }
    }
}
